...
class Provision(Register):
    ...
    def vcenter_connect(self):
        cert = "powershell Connect-VIServer -Server %s -Protocol https -User %s -Password %s;" \
                % (args_perf_esx['vcenter'], args_perf_esx['vcenter_user'], args_perf_esx['vcenter_passwd'])
        ssh_vcenter = {
            "host": args_perf_esx["vcenter"],
            "username": args_perf_esx["win_user"],
            "password": args_perf_esx["win_passwd"]
            }
        return cert, ssh_vcenter

    def esx_vms_list(self):
        cert, ssh_vcenter = self.vcenter_connect()
        cmd = "%s Get-VM |select Name" % (cert)
        ret, output = self.runcmd(cmd, ssh_vcenter, desc="vcenter guest list")
        vm_names = list()
    ...
    def esx_vms_add(self, index_start, index_end):
        time.sleep(10)
        for esxi_ip in args_perf_esx["esxi_hosts"].split(','):
            esxi_ip = esxi_ip.strip()
            logger.info("[%s] start to add guests" %(esxi_ip))
            ssh_esx = {
                "host": esxi_ip,
                "username": args_perf_esx["esxi_hosts_user"],
                "password": args_perf_esx["esxi_hosts_passwd"]
                }
            random_str = ''.join(map(lambda xx:(hex(ord(xx))[2:]),os.urandom(8)))[0:8]
            for vm_id in xrange(index_start, index_end):
                vm_name = "%s_vm%s" %(random_str, vm_id)
                cmd = "vim-cmd vmsvc/createdummyvm %s /vmfs/volumes/datastore*/" %vm_name
                ret, output = self.runcmd(cmd, ssh_esx, desc="add dummy guest %s" %vm_name)
                logger.info("create guest: %s" % vm_name)
        time.sleep(60)

    def esx_vms_del(self):
        cert, ssh_vcenter = self.vcenter_connect()
        for vm_name in self.esx_vms_list():
            cmd = "%s Remove-VM -VM %s -DeletePermanently -Confirm:$false" % (cert, vm_name.strip())
