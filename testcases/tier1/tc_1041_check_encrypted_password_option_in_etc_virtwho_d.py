...
class Testcase(Virtwho):
    def test_run(self):
        ...
        self.vw_case_info(case_name, "RHEL-133708")
        ...
        encrypted_value = self.vw_encrypted(password)
        self.vw_option_disable("password", filename=conf_file)
        self.vw_option_add("encrypted_password", encrypted_value, filename=conf_file)
        data, tty_output, rhsm_output = self.vw_start(exp_send=1)
        res = self.op_normal_value(data, exp_error=0, exp_thread=1, exp_send=1)
        results.setdefault('step2', []).append(res)
        self.vw_option_del("encrypted_password", filename=conf_file)
...
#- Case created to validate encrypted_password option
