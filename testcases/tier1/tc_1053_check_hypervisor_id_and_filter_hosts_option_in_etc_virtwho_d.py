...
class Testcase(Virtwho):
    def test_run(self):
        ...
        self.vw_case_info(case_name, "RHEL-133709")
        ...
        for step, option in sorted(steps.items(),key=lambda item:item[0]):
            ...
            self.vw_option_add("filter_hosts", hypervisorId, filename=conf_file)
            data, tty_output, rhsm_output = self.vw_start()
            s1 = self.op_normal_value(data, exp_error=0, exp_thread=1, exp_send=1)
            results.setdefault(step, []).append(s1)
            hypervisor_num = data[owner]['hypervisor_num']
            if data[owner].has_key(hypervisorId) and hypervisor_num == 1:
                logger.info("Succeeded to search, %s hypervisorId(%s) found" % (hypervisor_num, hypervisorId))
                results.setdefault(step, []).append(True)
            else:
                logger.error("Failed to search, %s hypervisorId(%s) found" % (hypervisor_num, hypervisorId))
...
#- Case created to validate hypervisor_id and filter_hosts option
