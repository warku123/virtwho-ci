...
class Testcase(Virtwho):
    def test_run(self):
        ...
        self.vw_case_info(case_name, "RHEL-133700")
        ...
        self.vw_option_del("filter_host_parents", filename=conf_file)

        logger.info(">>>step2: run virt-who with exclude_host_parents=[domain_id]")
        self.vw_option_add("exclude_host_parents", domain_id, filename=conf_file)
        data, tty_output, rhsm_output = self.vw_start()
        s1 = self.op_normal_value(data, exp_error=0, exp_thread=1, exp_send=1)
        ...
        self.vw_option_update_value("exclude_host_parents", "*", filename=conf_file)
        data, tty_output, rhsm_output = self.vw_start()
        s1 = self.op_normal_value(data, exp_error=0, exp_thread=1, exp_send=1)
...
#- Case created to validate exclude_host_parents option
