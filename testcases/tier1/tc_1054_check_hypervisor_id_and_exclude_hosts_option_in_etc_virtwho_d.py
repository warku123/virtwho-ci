...
class Testcase(Virtwho):
    def test_run(self):
        ...
        self.vw_case_info(case_name, "RHEL-133714")
        ...
        for step, option in sorted(steps.items(),key=lambda item:item[0]):
            logger.info(">>>%s: run virt-who to with hypervisor_id=%s and set exclude_hosts" % (step, option))
            ...
            self.vw_option_add("exclude_hosts", hypervisorId, filename=conf_file)
            data, tty_output, rhsm_output = self.vw_start()
            s1 = self.op_normal_value(data, exp_error=0, exp_thread=1, exp_send=1)
            results.setdefault(step, []).append(s1)
            s2 = self.vw_msg_search(str(data), hypervisorId, exp_exist=False)
...
#- Case created to validate hypervisor_id and exclude_hosts option
