...
class Testcase(Virtwho):
    def test_run(self):
        ...
        self.vw_case_info(case_name, "RHEL-134121")
        ...
        cmd1 = self.vw_cli_base(mode) + "-d -l %s" % log_dir
        cmd2 = self.vw_cli_base(mode) + "-d --log-dir %s" % log_dir
        steps = {'step1':cmd1, 'step2':cmd2}

        # Case Steps
        for step, cmd in sorted(steps.items(),key=lambda item:item[0]):
            logger.info(">>>%s: run virt-who cli to check log-dir" % step)
