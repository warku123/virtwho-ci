...
class Testcase(Virtwho):
    def test_run(self):
        ...
        self.vw_case_info(case_name, "RHEL-136631")
        ...
        self.vw_option_update_value("rhsm_password", "xxxxxx", conf_file)
        ...
        self.vw_option_update_value("rhsm_password", "红帽©¥®ðπ∉", conf_file)
        data, tty_output, rhsm_output = self.vw_start()
        res1 = self.op_normal_value(data, exp_error=1, exp_thread=1, exp_send=0)
        ...
        self.vw_option_update_value("rhsm_password", " ", conf_file)
        data, tty_output, rhsm_output = self.vw_start()
        res1 = self.op_normal_value(data, exp_error=0, exp_thread=1, exp_send=1)
        results.setdefault('step4', []).append(res1)

        logger.info(">>>step5: run virt-who with rhsm_password disable")
        self.vw_option_disable("rhsm_password", conf_file)
        data, tty_output, rhsm_output = self.vw_start()
        res1 = self.op_normal_value(data, exp_error=0, exp_thread=1, exp_send=1)
...
#- Case created to validate rhsm_password
