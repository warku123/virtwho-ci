...
class Testcase(Virtwho):
    def test_run(self):
        ...
        self.vw_case_info(case_name, "RHEL-136574")
        ...
        cli = self.vw_cli_base_update(base_cli, "--%s-password=.*" % mode, "--%s-password=xxxxx" % mode)
        ...
        cli = self.vw_cli_base_update(base_cli, "--%s-password=.*" % mode, "--%s-password=红帽€467aa" % mode)
        data, tty_output, rhsm_output = self.vw_start(cli)
        if "libvirt-remote" in mode or "libvirt" in mode:
            logger.warning("libvirt-remote can use sshkey to connect, password value is not necessary")
            res1 = self.op_normal_value(data, exp_error=0, exp_thread=1, exp_send=1)
        ...
        cli = self.vw_cli_base_update(base_cli, "--%s-password=.*" % mode, "--%s-password= " % mode)
        data, tty_output, rhsm_output = self.vw_start(cli)
        if "libvirt-remote" in mode or "libvirt" in mode:
        ...
        cli = self.vw_cli_base_update(base_cli, "--%s-password=.*" % mode, " ")
        data, tty_output, rhsm_output = self.vw_start(cli)
        if "libvirt-remote" in mode or "libvirt" in mode:
            logger.warning("libvirt-remote can use sshkey to connect, username is not necessary")
            res1 = self.op_normal_value(data, exp_error=0, exp_thread=1, exp_send=1)
...
# - Case created to validate password option
