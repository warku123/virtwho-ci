...
class Testcase(Virtwho):
    def test_run(self):
        ...
        self.vw_case_info(case_name, "RHEL-150338")
        ...
        for num in [30, 50]:
            ...
            for step in ["debug_on"]:
                logger.info("**********************************************************")
                logger.info("Run virt-who with: files(%s), oneshot_enable, %s" % (num, step))
                logger.info("**********************************************************")
                if step == "debug_on":
                    t1 = threading.Thread(target=self.perf_virtwho_start, kwargs = {"oneshot": True, "debug": True});
                if step == "debug_off":
                    t1 = threading.Thread(target=self.perf_virtwho_start, kwargs = {"oneshot": True, "debug": False});
